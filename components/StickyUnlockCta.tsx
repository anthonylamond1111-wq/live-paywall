...
export default function StickyUnlockCta({ visible, onUnlock }: StickyUnlockCtaProps) {
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-red-600/30 bg-black/95 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md">
      <div className={`${LANDING_FUNNEL_WIDTH} flex items-center gap-3 px-4 sm:px-0`}>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-white">Unlock full stream</p>
          <p className="truncate text-xs text-gray-500">{CHECKOUT_LABEL}</p>
        </div>
        <button
          type="button"
          onClick={onUnlock}
          className="shrink-0 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-black transition hover:bg-gray-100 active:scale-[0.985]"
        >
          Unlock
        </button>
      </div>
