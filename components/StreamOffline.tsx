...
export default function StreamOffline({ variant, fill = false, subtitle }: StreamOfflineProps) {
  ...
  const message =
    subtitle ??
    (variant === 'scheduled'
      ? `Goes live ${EVENT.streamStartLabel}. Free preview starts when we broadcast.`
      : 'The stream has not gone live yet. Keep this page open — it connects when broadcasting begins.');
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-black/40" aria-hidden />

      <div className="relative z-10 flex flex-col items-center">
        <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full border border-red-500/40 bg-black/60">
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-gray-300">{message}</p>
        ...
        {variant === 'scheduled' && countdown.isBeforeEvent && (
          <div className="mt-6">
            ...
            <div className="countdown-tick flex items-center justify-center gap-2 font-mono text-2xl font-bold text-white sm:gap-3 sm:text-3xl">
              {countdown.days > 0 && (
                <>
              ...
              <span className="text-red-500/50">:</span>
              <span className="countdown-seconds">{pad(countdown.seconds)}s</span>
            </div>
            <p className="mt-3 text-xs font-medium text-gray-400">{EVENT.streamStartLabel}</p>
          </div>
        )}
